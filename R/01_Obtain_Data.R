...
# records for Acraea terpsicore in Australia since 2012 - 2026 grouped by year and basis of record
...
occurrence_data<-galah_call() %>% 
  galah_identify("Acraea terpsicore") %>% 
  galah_filter(
    year>2012,
    country=="Australia"
  ) %>% 
...
write.csv(occurrence_data, file = "output/tcocc.csv")
